#!/usr/bin/env python3
"""
Auto-approval hook for Twenty CRM API operations.

Safe operations (auto-approved):
- All GET requests (list, get, metadata)
- check-auth.sh

Operations requiring approval:
- POST requests (create, link)
...
# Debug log path
DEBUG_LOG = os.path.expanduser("~/.config/twenty-crm/hook-debug.log")
...
def is_safe_operation(command: str) -> bool:
    ...
    Safe operations:
    - check-auth.sh
    - metadata/objects.sh, metadata/object.sh
    - */list.sh, */get.sh (read operations)

    Unsafe operations:
    - */create.sh, */update.sh, */delete.sh, */link.sh
    """
    # Patterns for safe operations (read-only)
    safe_patterns = [
        r"check-auth\.sh",
        r"metadata/objects\.sh",
        r"metadata/object\.sh",
        r"/list\.sh",
        r"/get\.sh",
        r"/me\.sh",
    ]
...
def main():
    ...
    try:
        ...
        # Check if this is a Twenty CRM related command
        if "twenty-crm" not in command and "twenty_crm" not in command:
            # Not a Twenty CRM command, let it go through normal flow
            sys.exit(0)
        ...
        if is_safe_operation(command):
            ...
            print(json.dumps({
                "hookSpecificOutput": {
                    ...
                    "decision": {
                        "behavior": "allow",
                        "message": "Auto-approved: Twenty CRM read operation"
                    }
                }
            }))
            sys.exit(0)
