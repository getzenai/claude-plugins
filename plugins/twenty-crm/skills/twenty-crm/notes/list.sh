#!/bin/bash
# List notes with optional filtering, ordering, and pagination
# Usage: list.sh [filter] [order_by] [limit] [depth]
# Example: list.sh 'title[ilike]:%meeting%' 'createdAt[DescNullsLast]' 50
...
URL="$BASE_URL/rest/notes?limit=$LIMIT&depth=$DEPTH"
